...
ui <- fluidPage(
  ...
  # Application title
  titlePanel("Reporting Time Series Data on Confirmed Cases in the US and 4 Other Countries"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("select_type", 
                  ...
                  selected = "Confirmed"),
      # Date range
      sliderInput("dates", label = "Report Date", min = first_date, max = last_date, value=c(first_date,last_date))
    ),
...
server <- function(input, output) {
  ...
  output$Plot1 <- renderPlot({
    ...
    pick_country <- global_time_series %>% 
      ...
      #Select Countries
      filter(Country_Region %in% c("US","Canada","Italy","China","Australia"))
    ...
    ggplot(pick_country, aes_string(x = "Date",  y = input$select_type, color = "Country_Region")) + 
      ...
      xlim(input$dates) +
      ggtitle("JHU COVID-19 Data on Confirmed Cases in the US and 4 Other Countries:", input$select_type)
  })
}
