...
report_03_24_2020 %>% 
  ...
  ggplot(aes(x = Deaths,  y = reorder(Province.State, Deaths))) + 
    ...
    ggtitle("The 15 provinces in china with the most reported COV19-related deaths") +
    ylab("Country/Region") +
    xlab("Deaths")
```
...
time_series_confirmed_long %>% 
  filter (Country.Region == "Canada") %>% 
    ...
    ggtitle("Canada Confirmed COVID-19 Cases")
```

```{r}
time_series_confirmed_long %>% 
  filter (Country.Region %in% c("China","Canada", "Korea, South",
                                "Italy","Australia", "US")) %>% 
    ggplot(aes(x = Date,  y = Confirmed)) + 
...
```{r}
time_series_confirmed_long %>% 
  filter (Country.Region %in% c("China","Canada","Italy", 
                                "Korea, South", "US")) %>% 
...
ggplot() +
  ...
  geom_point(data=time_series_confirmed, aes(x=Long, y=Lat, size=`3/25/20`, color=`3/25/20`),stroke=F, alpha=0.7) +
...
static <- ggplot(subset(time_series_Deaths_long2,Date %in% seq(min(Date),max(Date),7)),
            ...
            borders("world", colour = NA, fill = "gold") +
            theme_bw() +
